...
//Descripción del programa: Este programa calcula y muestra los términos de la serie de Taylor para sen(x) hasta un número dado de términos.
...
// Función para calcular un término de la serie de Taylor para sen(x).
float TermTaylor(int i, int x) {
    return (pow(-1, i) * pow(x, (2 * i) + 1) / DoFactorial((2 * i) + 1));
}

// Función recursiva para calcular la suma de los términos de la serie de Taylor para sen(x).
...
int main() {
    ...
    printf("Ingrese el numero de terminos que desea conocer de la serie de Taylor del sen(x): ");

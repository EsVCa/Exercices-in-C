...
//Descripción del programa: Este programa calcula y muestra los términos de la serie de Taylor para e^x hasta un número dado de términos. El usuario puede ingresar
// el número de términos y el valor de x.
...
// Función para calcular un término de la serie de Taylor.
float TermTaylor(int i, int x) {
    return pow(x, i) / DoFactorial(i);
}

// Función recursiva para calcular la suma de los términos de la serie de Taylor.
...
int main() {
    ...
    printf("Ingrese el numero de terminos que desea conocer de la serie de Taylor de e^x: ");

...
//Descripción del programa: Este programa calcula y muestra los términos de la serie de Taylor para cosh(x) hasta un número dado de términos.
...
// Función para calcular un término de la serie de Taylor para cosh(x).
float TermTaylor(int i, int x) {
    return pow(x, 2 * i) / DoFactorial(2 * i);
}

// Función recursiva para calcular la suma de los términos de la serie de Taylor para cosh(x).
...
int main() {
    ...
    printf("Ingrese el numero de terminos que desea conocer de la serie de Taylor del cosh(x): ");
